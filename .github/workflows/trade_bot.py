...
def main():
    ...
    if missing:
        print(f"❌ [0단계] GitHub Secret 누락: {', '.join(missing)}")
        print("   Settings → Secrets and variables → Actions 에서 이름을 정확히 확인하세요.")
        return 1
    print(f"✅ [0단계] 시크릿 3개 확인 (ALPACA_API_KEY 앞 4자: {ALPACA_KEY[:4]}..., "
          f"ANTHROPIC 앞 7자: {ANTHROPIC_KEY[:7]}...)")

    # ── 1단계: Alpaca 모의계좌 연결 테스트 ──
    try:
        account = alpaca("/v2/account")
        print(f"✅ [1단계] Alpaca 연결 성공 — 총자산 ${account['equity']}, 현금 ${account['cash']}")
    except urllib.error.HTTPError as e:
        print(f"❌ [1단계] Alpaca 인증 실패 (HTTP {e.code})")
        if e.code in (401, 403):
            print("   → 키가 틀렸거나, 모의계좌(Paper)가 아닌 키일 가능성이 커요.")
            print("   → 알파카 대시보드 왼쪽 위가 'Paper Trading'인 상태에서 키를 재생성하고,")
            print("     GitHub Secret 값을 새로 붙여넣어 주세요 (앞뒤 공백·따옴표 없이).")
        return 1
    except Exception as e:
        print(f"❌ [1단계] Alpaca 연결 오류: {e}")
        return 1
    ...
    try:
        ...
        try:
            ...
            pass
        print(f"❌ [2단계] Claude API 실패 (HTTP {e.code}) {detail}")
        if e.code == 401:
            print("   → API 키가 잘못됐어요. console.anthropic.com 에서 키를 재확인하세요.")
        elif e.code == 400 and "credit" in detail.lower():
            print("   → 크레딧 부족이에요. Billing에서 충전 상태를 확인하세요.")
        return 1
    except Exception as e:
        print(f"❌ [2단계] Claude API 오류: {e}")
        return 1
    ...
    for sym in watch:
        try:
            ...
            print(f"⚠️ {sym} 시세 실패: {e}")
    print(f"✅ [3단계] 시세 확보 {len(market)}/{len(watch)} 종목")
    ...
    try:
        ...
        print("✅ [4단계] Claude 판단 수신")
    except Exception as e:
        print(f"❌ [4단계] Claude 판단 실패: {e}")
