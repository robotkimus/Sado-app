...
TENBAGGER_AUTO_SCORE = 8                   # 이 점수 이상이 자동 편입 '결승 진출' 자격(부분점수제 만점 12)
...
def score_tenbagger_candidates(market):
    ...
    for m in market:
        ...
        if off is not None and off <= TENBAGGER_DEEP_DIP_PCT:
            ...
            score += 1; reasons.append(f"고점 대비 {off:.0f}% 조정")

        # ② 과매도 탈출 + 반등 신호 (단계 점수 — 타이밍 유연화)
        rsi = m.get("rsi14")
        ...
        if rsi is not None and rsi < 40 and price and ma5 and price >= ma5:
            ...
            score += 1; reasons.append(f"RSI {rsi:.0f} 과매도권 접근")
        ...
        if use_pe is not None and use_pe < 15:
            ...
            score += 1; reasons.append(f"PER {use_pe:.0f}")
        if pb is not None:
            max_possible += 1
        if pb is not None and 0 < pb < 1.5:
            score += 1; reasons.append(f"PBR {pb:.1f}")
        # 그레이엄 복합 기준: PER × PBR ≤ 22.5 (둘 다 있을 때)
        if use_pe is not None and pb is not None:
            max_possible += 1
            if 0 < use_pe * pb <= 22.5:
                score += 1; reasons.append("그레이엄 기준 충족")

        # ③-2 우량성(버핏 ROE): ROE 15% 이상 (데이터 있을 때만 — 없으면 만점에서 제외해 공정 판정)
        roe = val.get("roe")
        if roe is not None:
            max_possible += 2
            if roe >= 15:
        ...
        # 획득 가능 점수 대비 비율 (데이터 누락 종목도 공정하게 평가)
        ratio = round(score / max_possible, 2) if max_possible >= 6 else 0

        if score >= 4:   # 4점 이상만 후보로 (4박자 중 2개 이상 강하게 충족)
            cands.append({"symbol": sym, "score": score, "ratio": ratio,
                          "max": max_possible,
...
def auto_enroll_tenbagger(candidates):
    ...
    # 결승 진출: 8점 이상 또는 '획득가능 대비 75%+'(데이터 누락 종목 구제, 최소 6점)
    finalists = [c for c in candidates
                 if (c["score"] >= TENBAGGER_AUTO_SCORE
                     or (c.get("ratio", 0) >= 0.75 and c["score"] >= 6))
